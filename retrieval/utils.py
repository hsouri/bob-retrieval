...
def extract_features(args, model, data_loader, use_cuda=True, multiscale=False):
    ...
    for samples, index in metric_logger.log_every(data_loader, 100):
        ...
        if multiscale:
            ...
            if args.pt_style in ['clip']:
                
                if 'vit' in args.arch:
                    feats = model.module.forward_features(samples)[:,0,:].clone()
                else:
                    feats = model.module.forward_features(samples).clone()
            elif args.pt_style in ['mae', 'moco']:
                feats = model.module.forward_features(samples).clone()
...
def micro_average_precision(gnds, probas_pred):
    ...
    microAPs = []
    for i in range(num):
        y_true = np.zeros_like(probas_pred[i,:])
        loca = gnds[i]['ok']
        y_true[loca] = 1
...
#### code for GPR1200 dataset class
import os
# import pandas as pd
...
class GPRdataset(Dataset):
    def __init__(self, img_dir,split, transform=None,numvals = 4):
        ...
        filenames = [x for x in os.listdir(img_dir) if x.endswith(('.JPG', '.JPEG', '.jpg'))]
# filenames = sorted(filenames)
        if split == 'query':
            self.namelist = []
            for j in range(1200):
                temp = [file for file in filenames if file.startswith(f'{j}_')]
                self.namelist.append(temp[0])
        elif split == 'value':
            self.namelist = []
            num = self.numvals+1
            for j in range(1200):
                temp = [file for file in filenames if file.startswith(f'{j}_')]
                if num > len(temp):
                    self.namelist.append(temp[1:])
                else:
                    self.namelist.append(temp[1:num])
...
class TwoLeveldataset(Dataset):
    def __init__(self, img_dir,split, transform=None,maxlim = None, qvs=0.2):
        self.img_dir = img_dir
        self.split = split
        self.transform = transform
        # self.numvals = numvals
        ...
        if split == 'query':
            ...
            for dir in flevel_dirs:
                ...
                temp = [x for x in os.listdir(cdir) if x.endswith(('.JPG', '.JPEG', '.jpg', '.png', '.PNG'))]
                temp = sorted(temp)
                if maxlim is not None:
                    temp = temp[:maxlim]
                till = int((len(temp)*qvs)//1)
                qfiles = [f"{dir}/{temp[i]}" for i in range(till)]
                self.namelist.append(qfiles)
                for key in range(start_q,start_q+till):
                    self.newcfg[key] = {
                        'ok': list(range(start_val, start_val + len(temp)-till)),
                        'junk':[]
                    }
                start_q = start_q+till
                start_val = start_val + len(temp)-till
        elif split == 'value':
            # import ipdb; ipdb.set_trace()
            self.namelist = []
            # num = self.numvals+1
            for dir in flevel_dirs:
                ...
                if maxlim is not None:
                    temp = temp[:maxlim]
                till = int((len(temp)*qvs)//1)
                val_files =  [f"{dir}/{word}" for word in temp[till:]]
...
class Copydaysdataset(Dataset):
    def __init__(self, img_dir,split, transform=None):
        self.img_dir = img_dir
        self.split = split
        self.transform = transform
        with open(f"{img_dir}/qvmapping.p", 'rb') as f:
        ...
        for i,tq in enumerate(queries):
            mapped_vals = mapping[tq]
            loca = np.argwhere(np.isin(vals, mapped_vals)).ravel()
            self.qvmapping[i] = {
                'ok': list(loca),
                'junk' : []
            }
        ...
        if split == 'query':
            ...
            self.namelist = []
            for q in queries:
                self.namelist.append(f"jpg/{q}")   
                
        elif split == 'value':
            self.namelist = []
            for v in vals:
                self.namelist.append(f"jpg/{v}") 
...
class GoogleLMRet(Dataset):
    def __init__(self, img_dir,split, transform=None):
        self.img_dir = img_dir
        self.split = split
        self.transform = transform
        df = pd.read_csv(f"{img_dir}/retrieval_solution_v2.1.csv")
        df = df[df['images']!= "None"]
        mapping = dict(zip(df['id'], df['images']))
        self.newcfg = {}
        q_files = [x for x in os.listdir(f"{self.img_dir}/query") if x.endswith(('.JPG', '.JPEG', '.jpg', '.png', '.PNG'))]
        q_files =  natsort.natsorted(q_files)
        v_files = [x for x in os.listdir(f"{self.img_dir}/database") if x.endswith(('.JPG', '.JPEG', '.jpg', '.png', '.PNG'))]
        v_files =  natsort.natsorted(v_files)
        if split == 'query':
            self.namelist = q_files
            for i in range(len(q_files)):
                ...
                vf = mapping[qf]
                vf = vf.split(' ')
                vf = [f"{x}.jpg" for x in vf]
                
                self.newcfg[i] = {
                    'ok': [v_files.index(x) for x in vf],
                    'junk':[]
                }
            self.img_dir = f"{self.img_dir}/query"
                
        elif split == 'value':
            self.namelist = v_files
            self.img_dir = f"{self.img_dir}/database"
    def __len__(self):
        return len(self.namelist)
